  ...
  if (isEditing) {
    return (
      <div className="bg-[#4a3f37] w-full p-4 rounded-xl border border-beigeLight/50 mb-3 space-y-2 text-left shadow-lg">
        ...
        <input name="name" type="text" value={editForm.name} onChange={onFormChange} className="w-full p-2 rounded bg-gray-800 text-white text-sm border border-brownBorder outline-none" />
        <label className="text-[10px] text-gray-400 block px-1">사유</label>
        <input name="reason" type="text" value={editForm.reason} onChange={onFormChange} className="w-full p-2 rounded bg-gray-800 text-white text-sm border border-brownBorder outline-none" />
        <label className="text-[10px] text-gray-400 block px-1">금액</label>
        <input name="amount" type="text" value={editForm.amount} onChange={onFormChange} className="w-full p-2 rounded bg-gray-800 text-white text-sm border border-brownBorder outline-none" />
  ...
  return (
    <div className="bg-[#4a3f37] w-full p-4 rounded-xl border border-brownBorder/30 mb-3 shadow-sm text-left">
      ...
      <div className="flex gap-2 border-t border-brownBorder/20 pt-3">
        <button onClick={() => onStartEdit(item, listType)} className="flex-1 py-1.5 rounded bg-gray-600 hover:bg-gray-500 text-white text-[10px] font-bold">수정</button>
        <button onClick={() => onDelete(item.id, listType)} className="flex-1 py-1.5 rounded bg-rose-900/50 hover:bg-rose-800 text-rose-200 text-[10px] font-bold transition border border-rose-800/30">삭제</button>
      </div>
    </div>
  );
};
  ...
  const refreshSearch = async (isAll = false) => {
    ...
    try {
      ...
      if (isAll) {
        ...
        allResults = snapshot.docs.map((doc) => ({ id: doc.id, ...(doc.data() as Omit<DonationData, "id">) }));
      } else {
      ...
      <div className="w-full max-w-md space-y-3 mb-10">
        <div className="flex gap-2">
          <input
            type="text"
            placeholder="이름으로 검색"
            className="flex-1 p-3 border border-brownBorder rounded-2xl bg-gray-700 text-white placeholder-gray-400 outline-none focus:ring-2 focus:ring-beigeLight shadow-inner"
            value={searchName}
            onChange={(e) => setSearchName(e.target.value)}
            onKeyDown={(e) => e.key === 'Enter' && refreshSearch(false)}
          />
          <button onClick={() => refreshSearch(false)} className={`px-6 rounded-2xl font-black transition-all active:scale-95 shadow-lg ${searchName ? "bg-beigeLight text-darkText" : "bg-gray-600 text-gray-400 cursor-not-allowed"}`} disabled={!searchName || loading}>{loading ? "..." : "검색"}</button>
        </div>
        {typeof window !== "undefined" && localStorage.getItem("userId") === "admin" && (
          <button onClick={() => refreshSearch(true)} className="w-full py-3 bg-[#4F7A6E] text-white font-bold rounded-xl shadow-md text-sm">📂 전체 내역 불러오기 (admin 전용)</button>
        )}
      </div>
        ...
        <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="flex flex-col">
            <div className="flex items-center justify-between mb-4 px-2">
              <h3 className="text-beigeLight font-black flex items-center gap-2 text-sm">📥 입금 (받은 내역)</h3>
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => { setAddingType("received"); setForm({ date: getTodayStr(), name: searchName, reason: "", amount: "" }); }} className="bg-beigeLight text-darkText w-6 h-6 rounded-full font-black text-lg flex items-center justify-center shadow-md active:scale-90 transition">+</button>
              </div>
            </div>

            {addingType === "received" && (
              <div className="bg-[#322c26] p-4 rounded-xl border border-beigeLight/30 mb-4 space-y-2 shadow-lg text-left">
                <p className="text-[10px] font-bold text-beigeLight mb-1 uppercase tracking-wider">✨ 받은 내역 추가 등록</p>
                ...
                <input name="name" type="text" value={form.name} onChange={handleFormChange} className="w-full p-2 rounded bg-gray-800 text-white text-xs border border-brownBorder" placeholder="이름" />
            ...
            <div className="space-y-1">
              ...
              {receivedResults.length === 0 && !addingType && <p className="text-gray-500 text-center py-10 text-xs border border-dashed border-gray-700 rounded-xl">내역이 없습니다.</p>}
            </div>
          </div>
          ...
          <div className="flex flex-col">
            <div className="flex items-center justify-between mb-4 px-2">
              <h3 className="text-[#86b5a7] font-black flex items-center gap-2 text-sm">📤 출금 (보낸 내역)</h3>
              <div className="flex items-center gap-2">
                <span className="text-[10px] bg-[#4F7A6E]/20 text-[#86b5a7] px-2 py-1 rounded-full font-bold">{sentResults.length}건</span>
                <button onClick={() => { setAddingType("sent"); setForm({ date: getTodayStr(), name: searchName, reason: "", amount: "" }); }} className="bg-[#4F7A6E] text-white w-6 h-6 rounded-full font-black text-lg flex items-center justify-center shadow-md active:scale-90 transition">+</button>
              </div>
            </div>

            {addingType === "sent" && (
              <div className="bg-[#322c26] p-4 rounded-xl border border-beigeLight/30 mb-4 space-y-2 shadow-lg text-left">
                <p className="text-[10px] font-bold text-beigeLight mb-1 uppercase tracking-wider">✨ 보낸 내역 추가 등록</p>
                <input name="date" type="date" value={form.date} onChange={handleFormChange} className="w-full p-2 rounded bg-gray-800 text-white text-xs border border-brownBorder" />
                <input name="name" type="text" value={form.name} onChange={handleFormChange} className="w-full p-2 rounded bg-gray-800 text-white text-xs border border-brownBorder" placeholder="이름" />
                <input name="reason" type="text" value={form.reason} onChange={handleFormChange} className="w-full p-2 rounded bg-gray-800 text-white text-xs border border-brownBorder" placeholder="사유" />
                <input name="amount" type="text" value={form.amount} onChange={handleFormChange} className="w-full p-2 rounded bg-gray-800 text-white text-xs border border-brownBorder" placeholder="금액" />
            ...
            <div className="space-y-1">
              ...
              {sentResults.length === 0 && !addingType && <p className="text-gray-500 text-center py-10 text-xs border border-dashed border-gray-700 rounded-xl">내역이 없습니다.</p>}
            </div>
          </div>
        </div>
      )}
